...
- **자율모드** 체크는 시작 스위치가 아니라 동작 방식 설정입니다. 감시가 켜졌을 때 `--mode auto`로 입력대기와 명시적으로 요청된 `<<DONE>>` 작업만 계속할지 정합니다.
...
4. 정상 완료/유휴 세션은 깨우지 않습니다. 최근 사용자 지시에 `<<DONE>>` 계약이 있을 때만 `done` 상태에서 sentinel 미출력을 계속할 작업으로 봅니다.
5. 대상이 확실하면 nudge를 보내고, 불확실하면 알림과 ledger 기록만 남깁니다.
...
- 최근 사용자 지시에 `<<DONE>>` 완료 계약이 없는 정상 종료 세션은 자율모드에서도 키를 보내지 않습니다.
