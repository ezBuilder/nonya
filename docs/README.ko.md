...
다운로드: [v0.2.5 릴리즈](https://github.com/ezBuilder/nonya/releases/tag/v0.2.5)

## 장점

- 밤새 멈춘 세션을 감지하고 복구 시도 또는 알림으로 회수합니다.
- 자율 모드에서 입력대기 질문을 로컬 지침 또는 안전한 기본 응답으로 처리합니다.
- `<<DONE>>` 계약이 없는 정상 완료/유휴 세션은 자율 모드에서도 깨우지 않습니다.
...
- **자율모드** 체크는 시작 스위치가 아니라 감시 중 입력대기와 명시적으로 요청된 `<<DONE>>` 작업만 계속할지 정하는 동작 방식입니다.
